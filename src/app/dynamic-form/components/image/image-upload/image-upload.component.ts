...
import { ImageDescription, ImageItem, UploadConfig, ResourceProvider } from '../../../dynamic-form.options';
...
export class ImageUploadComponent implements OnInit {
  ...
  selectedFileOnChanged(event: any) {
    if (this.uploader.queue.length === 0) {
      this.alert(`选择图片失败（文件类型错误/文件大小超过` + this.config.maxFileSize / 1024 / 1024 + `M）`);
      return ;
    }
      ...
      queue.onSuccess = function (response, status, headers) {
        ...
        if (status === 200) {
          ...
          try {
            ...
            $this.alert(`上传图片失败（系统维护中）（ ` + queue._file.name.substring(0, 20) + ` ）`);
          }
        } else {
          $this.alert(`上传图片失败（文件尺寸太大/类型错误）（ ` + queue._file.name.substring(0, 20) + ` ）`);
        }
      };
      queue.onError = function (response, status, headers) {
        if (status === 401) {
          $this.alert(`上传图片失败（授权异常）（可关闭浏览器重新打开）`);
        } else {
          $this.alert(`上传图片失败（系统繁忙）（ ` + queue._file.name.substring(0, 20) + ` ）`);
        }
      };
