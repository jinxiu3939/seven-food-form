...
export class DynamicFormComponent implements OnInit {
  ...
  @Input() submitting: boolean; // 表单状态
  @Input() layout: string; // 布局

  @Output() public submit = new EventEmitter<any>(); // 表单提交事件
  @Output() public reset = new EventEmitter<boolean>(); // 表单重置事件
  ...
  /**
   * 表单提交
   * 父组件在提交结束之后，如果需要重新提交，必须设置`submitting`为`false`
   */
  onSubmit() {
    this.submit.emit(this.form.value); // 发送提交信息
  }
